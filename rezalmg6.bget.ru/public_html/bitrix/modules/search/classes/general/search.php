			...
			if($ar["DO_INSERT"])
			{
				...
				if($rsUpdate->AffectedRowsCount() <= 0)
				{
					$strSql = "
						...
						VALUES
						('".$DB->ForSQL($stem)."', '".$sql_lang_id."', ".(strlen($site_id) > 0? "'".$sql_site_id."'": "NULL").", ".$FREQ.", ".number_format($ar["TF"], 2, ".", "").")
					";
...
class CSearchSQLHelper
{
	...
	function _CallbackURL($field_name, $operation, $field_value)
	{
		...
		{
			$strInc = "";
			foreach($field_value as $url_i)
			{
				if(strlen($strInc)>0)
					$strInc .= " OR ";
				$strInc .= "(".$this->strSearchContentAlias.".URL LIKE '".$DB->ForSql($url_i)."' OR scsite.URL LIKE '".$DB->ForSql($url_i)."')";
			}
		...
		{
			$strSql = "(".$this->strSearchContentAlias.".URL LIKE '".$DB->ForSql($field_value)."' OR scsite.URL LIKE '".$DB->ForSql($field_value)."')";
			$this->bIncSites = true;
		}
