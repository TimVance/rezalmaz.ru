...
function __HSC_back($mxVar)
{
	if (true == is_array($mxVar))
	{
		foreach ($mxVar as $key => $value)
		{
			$mxVar[$key] = htmlspecialcharsback($value);
		}
	}
	else
	{
		$mxVar = htmlspecialcharsback($mxVar);
	}
	return $mxVar;
}
...
{
	$intIBlockID = htmlspecialchars($_REQUEST["PARAMS"]['IBLOCK_ID']);
	if (0 >= intval($intIBlockID))
		$boolFlag = false;
}
if (false == $boolFlag)
{
...
if (true == isset($_REQUEST['saveresult']))
{
	...
	foreach ($arFieldsList as $strFieldName)
		...
		{
			if (true == isset($_POST['PROPERTY_'.$strFieldName]))
			{
				$arProperty[$strFieldName] = htmlspecialchars($_POST['PROPERTY_'.$strFieldName]);
			}
			else
...
<input type="hidden" name="propedit" value="<? echo htmlspecialchars($str_PROPERTY_ID) ?>">
...
<tr>
	<td width="40%">ID:</td>
	<td width="60%"><? echo (0 < intval($arProperty['ID']) ? $arProperty['ID'] : GetMessage("IB_E_PROP_NEW"))?></td>
</tr>
<tr>
	<td><label for="PROPERTY_ACTIVE_Y"><?echo GetMessage("IB_E_PROP_ACT")?></label></td>
	<td><input type="hidden" id="PROPERTY_ACTIVE_N" name="PROPERTY_ACTIVE" value="N">
...
<tr>
	...
	<td ><input type="text" size="30" maxlength="50" id="PROPERTY_NAME" name="PROPERTY_NAME" value="<?echo htmlspecialchars($arProperty['NAME']);?>"></td>
</tr>
<tr>
...
<tr>
	<td><label for="PROPERTY_MULTIPLE_Y"><?echo GetMessage("IB_E_PROP_MULTIPLE")?></label></td>
	<td>
...
<tr>
	<td><label for="PROPERTY_IS_REQUIRED_Y"><?echo GetMessage("IB_E_PROP_IS_REQUIRED")?></label></td>
	<td>
...
if(true == $bShow)
{?><tr>
	<td><label id="PROPERTY_SEARCHABLE_Y"><?echo GetMessage("IB_E_PROP_SEARCHABLE")?></label></td>
	<td>
...
if(true == $bShow)
{?><tr>
	<td><label for="PROPERTY_FILTRABLE_Y"><?echo GetMessage("IB_E_PROP_FILTRABLE")?></label></td>
	<td>
...
if(true == $bShow)
{?><tr>
	<td><label for="PROPERTY_WITH_DESCRIPTION_Y"><?echo GetMessage("IB_E_PROP_WITH_DESC")?></label></td>
	<td>
...
{?><tr>
	...
	<td>
		<select id="PROPERTY_LIST_TYPE" name="PROPERTY_LIST_TYPE">
			<option value="L"<?if($arProperty['LIST_TYPE']!="C")echo " selected"?>><?echo GetMessage("IB_E_PROP_APPEARANCE_LIST")?></option>
			<option value="C"<?if($arProperty['LIST_TYPE']=="C")echo " selected"?>><?echo GetMessage("IB_E_PROP_APPEARANCE_CHECKBOX")?></option>
		</select>
	</td>
</tr>
<?
...
<input type="hidden" id="PROPERTY_ROW_COUNT" name="PROPERTY_ROW_COUNT" value="<?echo htmlspecialchars($arPropertyFields["SET"]["ROW_COUNT"])?>">
	<?}
?><tr class="heading"><td valign="top" colspan="2"><?echo GetMessage("IB_E_PROP_LIST_VALUES")?></td></tr>
<tr>
...
/*		if(0 < intval($arProperty['ID']))
		{
			$props = CIBlockProperty::GetPropertyEnum($arProperty['ID']);
	...
	<input type="button"  name="propedit_add" value="<?echo GetMessage("IB_E_PROP_LIST_MORE")?>" onclick="add_list_row()">
	</td>
</tr><?
}
...
<tr>
	...
	<td>
		...
		<select  onchange="if(this.selectedIndex!=0) document.getElementById('CURRENT_PROPERTY_FILE_TYPE').value=this[this.selectedIndex].value">
			<option value="-"></option>
			<option value=""<?if('' == $arProperty['FILE_TYPE'])echo " selected"?>><?echo GetMessage("IB_E_PROP_FILE_TYPES_ANY")?></option>
			<option value="jpg, gif, bmp, png, jpeg"<?if("jpg, gif, bmp, png, jpeg" == $arProperty['FILE_TYPE'])echo " selected"?>><?echo GetMessage("IB_E_PROP_FILE_TYPES_PIC")?></option>
			<option value="mp3, wav, midi, snd, au, wma"<?if("mp3, wav, midi, snd, au, wma" == $arProperty['FILE_TYPE'])echo " selected"?>><?echo GetMessage("IB_E_PROP_FILE_TYPES_SOUND")?></option>
			<option value="mpg, avi, wmv, mpeg, mpe"<?if("mpg, avi, wmv, mpeg, mpe" == $arProperty['FILE_TYPE'])echo " selected"?>><?echo GetMessage("IB_E_PROP_FILE_TYPES_VIDEO")?></option>
			<option value="doc, txt, rtf"<?if("doc, txt, rtf" == $arProperty['FILE_TYPE'])echo " selected"?>><?echo GetMessage("IB_E_PROP_FILE_TYPES_DOCS")?></option>
		</select>
	</td>
</tr>
<?
...
elseif("G" == $arProperty['PROPERTY_TYPE'] || "E" == $arProperty['PROPERTY_TYPE'])
{
	...
	<tr>
		...
		<td>
		<?
		$b_f == ($arProperty['PROPERTY_TYPE']=="G" ? array("!ID"=>$PARAMS['IBLOCK_ID']) : array());
...
{?><tr class="heading"><td colspan="2"><?echo (true == isset($arPropertyFields["USER_TYPE_SETTINGS_TITLE"])
												 && '' != trim($arPropertyFields["USER_TYPE_SETTINGS_TITLE"])
												 ? $arPropertyFields["USER_TYPE_SETTINGS_TITLE"] : GetMessage("IB_E_PROP_USER_TYPE_SETTINGS"));
	?></td></tr><?
...
function add_list_row()
{
	...
	var typeHtml = '<? echo CUtil::JSEscape(__AddListValueIDCell('ntmp_xxx',$arDefPropInfo)); ?>';
	typeHtml = typeHtml.replace(/tmp_xxx/ig, id);
	oCell.innerHTML = typeHtml;
	var oCell = newRow.insertCell(-1);
	var typeHtml = '<? echo CUtil::JSEscape(__AddListValueXmlIDCell('ntmp_xxx',$arDefPropInfo)); ?>';
	typeHtml = typeHtml.replace(/tmp_xxx/ig, id);
	oCell.innerHTML = typeHtml;
	var oCell = newRow.insertCell(-1);
	var typeHtml = '<? echo CUtil::JSEscape(__AddListValueValueCell('ntmp_xxx',$arDefPropInfo)); ?>';
	typeHtml = typeHtml.replace(/tmp_xxx/ig, id);
	oCell.innerHTML = typeHtml;
	var oCell = newRow.insertCell(-1);
	var typeHtml = '<? echo CUtil::JSEscape(__AddListValueSortCell('ntmp_xxx',$arDefPropInfo)); ?>';
	typeHtml = typeHtml.replace(/tmp_xxx/ig, id);
	oCell.innerHTML = typeHtml;
	var oCell = newRow.insertCell(-1);
	var typeHtml = '<? echo CUtil::JSEscape(__AddListValueDefCell('ntmp_xxx',$arDefPropInfo)); ?>';
	typeHtml = typeHtml.replace(/tmp_xxx/ig, id);
	oCell.innerHTML = typeHtml;
}
<?
