...
while($arRes = $rsData->NavNext(true, "f_"))
{
	...
	$edit_url = '/bitrix/admin/iblock_subelement_edit.php?WF=Y&type='.$strSubIBlockType.'&IBLOCK_ID='.$intSubIBlockID.'&lang='.LANGUAGE_ID.'&PRODUCT_ID='.$ID.'&ID='.$arRes_orig['ID'].'&TMP_ID='.$strSubTMP_ID.$sThisSectionUrl;
			...
			if($arRes_orig['WF_NEW']=="Y") // not published, under workflow
			{
				$arActions[] = array(
					...
					"ACTION"=>"javascript:(new BX.CAdminDialog({
			    		'content_url': '/bitrix/admin/iblock_subelement_edit.php?WF=Y&type=".$strSubIBlockType."&IBLOCK_ID=".$intSubIBlockID."&lang=".LANGUAGE_ID."&PRODUCT_ID=".$ID."&ID=".$arRes_orig['ID']."&TMP_ID=".urlencode($strSubTMP_ID).$sThisSectionUrl."', 
				...
				$arActions[] = array(
					...
					"ACTION"=>"javascript:(new BX.CAdminDialog({
			    		'content_url': '/bitrix/admin/iblock_subelement_edit.php?WF=Y&type=".$strSubIBlockType."&IBLOCK_ID=".$intSubIBlockID."&lang=".LANGUAGE_ID."&PRODUCT_ID=".$ID."&ID=".$arRes_orig['ID']."&TMP_ID=".urlencode($strSubTMP_ID)."&action=copy".$sThisSectionUrl."', 
			    		'content_post': '&bxpublic=Y',
						'draggable': true,
						'resizable': true,
						'buttons': [BX.CAdminDialog.btnSave, BX.CAdminDialog.btnCancel]
...
function ShowNewOffer(id)
{
	...
	{
		(new BX.CAdminDialog({
		    'content_url': '/bitrix/admin/iblock_subelement_edit.php?WF=Y&type=<? echo $strSubIBlockType; ?>&IBLOCK_ID=<? echo $intSubIBlockID; ?>&lang=<? echo LANGUAGE_ID; ?>&PRODUCT_ID=<? echo $intSubPropValue; ?>&ID=0&TMP_ID=<? echo urlencode($strSubTMP_ID).$sThisSectionUrl; ?>', 
