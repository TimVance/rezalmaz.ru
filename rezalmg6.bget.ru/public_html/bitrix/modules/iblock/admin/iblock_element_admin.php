	...
	foreach($arID as $ID)
	{
		...
		case "wf_status":
			if($bWorkFlow)
			{
				...
				if($new_status > 0 && CIBlockElement::WF_GetStatusPermission($new_status) > 0)
				{
				...
				{
					$lAdmin->AddGroupError(GetMessage("IBEL_A_SAVE_ERROR", array("#ID#" => $ID, "#ERROR_MESSAGE#" => GetMessage("IBEL_A_ACCESS_DENIED_STATUS")." [".$new_status."].<br>")), $TYPE.$ID);
				}
			}
			break;
		case "lock":
...
if($bWorkFlow)
{
	...
	$arActions["wf_status"] = GetMessage("IBEL_A_WF_STATUS_CHANGE");
...
{
	<?=$sTableID."_filter";?>.OnSet('<?=CUtil::AddSlashes($sTableID)?>', '<?=CUtil::AddSlashes($APPLICATION->GetCurPage().'?type='.$type.'&IBLOCK_ID='.$IBLOCK_ID.'&lang='.LANG.'&')?>');
	return false;
}
...
function deleteFilter()
{
	...
	<?=$sTableID."_filter"?>.OnClear('<?=CUtil::AddSlashes($sTableID)?>', '<?=CUtil::AddSlashes($APPLICATION->GetCurPage().'?type='.$type.'&IBLOCK_ID='.$IBLOCK_ID.'&lang='.LANG.'&')?>');
	return false;
}
</script><?php
$oFilter->Begin();
...
<tr>
	...
	<td>
		...
		<?
		elseif($arProp["PROPERTY_TYPE"]=='G'):
			echo _ShowGroupPropertyField('find_sub_el_property_'.$arProp["ID"], $arProp, ${'find_sub_el_property_'.$arProp["ID"]});
		endif;
		?>
	</td>
</tr>
