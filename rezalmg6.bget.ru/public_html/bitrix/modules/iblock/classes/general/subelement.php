...
/*			$link = DeleteParam(array("mode"));
	...
	function ActionAjaxReload($url)
	{
		...
		//return $this->table_id.".ActivateMainForm();\n".$this->table_id.".GetAdminList('".CUtil::AddSlashes($url)."');";
	...
	{
		global $APPLICATION;
		$strParams = "SUB_ID=".CUtil::AddSlashes($id)."&action=".CUtil::AddSlashes($action_id)."&lang=".LANGUAGE_ID."&".bitrix_sessid_get().($add_params<>""?"&".CUtil::AddSlashes($add_params):"");
		$strUrl = CUtil::AddSlashes($this->GetListUrl(true)).('' != $this->strListUrlParams ? '&' : '?').$strParams;
		return $this->table_id.".GetAdminList('".$strUrl."');";
	}
