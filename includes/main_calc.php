...
$calc_groups = CIBlockElement::GetList(Array("SORT"=>"ASC"), Array("IBLOCK_ID" => 3, "ACTIVE" => "Y"));
...
$calc_types = CIBlockElement::GetList(Array("SORT"=>"ASC"), Array("IBLOCK_ID" => 2, "ID" => $arr_types_list, "ACTIVE" => "Y"));
...
<div class="text_style calc-block">
    <div class="calc">
        <noindex>
            ...
            <?php endforeach;?>
            <div work="17" name="param" class="stage_3">
                <div class="stage">
                    <div class="column_short">
...
<!--                            <input type="text" class="changed_param" id="17_thick" value="0"> -->
                            <select class="changed_param" id="17_thick"></select>
                        </div>
                        <div class="sel_param_row">
                            <span class="label_param">Длина реза (м.)</span>
                            <input type="text" class="changed_param" id="17_length" value="0">
                        </div>
                    </div>
                </div>
            </div>
            <div work="18" name="param" class="stage_3">
                <div class="stage">
                    <div class="column_short">
                        <div class="text_head"><div class="step_index">3.</div>Укажите параметры:</div>
                    </div>
...
<!--                            <input type="text" class="changed_param" id="18_diameter" value="0"> -->
                            <select class="changed_param" id="18_diameter"></select>
                        </div>
                        <div class="sel_param_row">
                            <span class="label_param">Глубина сверления (см.)</span>
                            <input type="text" class="changed_param" id="18_depth" value="0">
                        </div>
                        <div class="sel_param_row">
                            <span class="label_param">Колличество отверстий</span>
                            <input type="text" class="changed_param" id="18_count" value="0">
                        </div>
                    </div>
                </div>
            </div>
            <div work="19" name="param" class="stage_3">
                <div class="stage">
                    <div class="column_short">
                        <div class="text_head"><div class="step_index">3.</div>Укажите параметры:</div>
                    </div>
                    <div class="column">
                        <div class="sel_param_row sel_padding">
                            ...
                            <div><input type="text" class="changed_param" id="19_wall" value="0"></div>
    ...
    ;(function ($, location) {
        $(function () {
            ...
            $('[name="material"]').on('change', function(event){
                ...
                if(parseInt($('[name="material"]:checked').val()) == 16){
                ...
                switch($(this).val()){
                    case '10':
                        ...
                        for (var key in obj){
                            ...
                            $('#17_thick').append('<option value="'+ obj[key] + '">' + key_z + '</option>');
                        }
                        break;
                    case '12':
                        $('#17_thick').empty();
                    ...
                    case '13':
                        ...
                        for (var key in obj){
                            var key_z = key.replace(/\*/g, '');
                            $('#18_diameter').append('<option value="'+ obj[key] + '">' + key_z + '</option>');
                        }
                        break;
                    case '14':
                        $('#18_diameter').empty();
...
//          });
            ...
            $('#calc_summ').on('click', function(event){
                ...
                switch($('[name="work"]:checked').val()){
                    case '17':
                        var thick = (parseFloat($('#17_thick').find('option:selected').text()) > 0) ? parseFloat($('#17_thick').find('option:selected').text()) : 0;
                        var price = (parseFloat($('#17_thick').find('option:selected').val()) > 0) ? parseFloat($('#17_thick').find('option:selected').val()) : 0;
                        var length = (parseFloat($('#17_length').val()) > 0) ? parseFloat($('#17_length').val()) : 0;
                        ...
                        break;
                    case '18':
                        var diameter = (parseFloat($('#18_diameter').find('option:selected').text()) > 0) ? parseFloat($('#18_diameter').find('option:selected').text()) : 0;
                        var price = (parseFloat($('#18_diameter').find('option:selected').val()) > 0) ? parseFloat($('#18_diameter').find('option:selected').val()) : 0;
                        var depth = (parseFloat($('#18_depth').val()) > 0) ? $('#18_depth').val() : 0;
                        var count = (parseInt($('#18_count').val()) > 0) ? $('#18_count').val() : 0;
                        ...
                        break;
                    case '19':    // ШТРОБЛЕНИЕ
                        var wall = (parseFloat($('#19_wall').val()) > 0) ? $('#19_wall').val() : 0;
                        var socket = (parseInt($('#19_socket').val()) > 0) ? $('#19_socket').val() : 0;
                        var cond = (parseInt($('#19_cond').val()) > 0) ? $('#19_cond').val() : 0;
                        
                        if(parseInt($('[name="material"]:checked').val()) == 16){
                            var ceiling = (parseFloat($('#19_ceiling').val()) > 0) ? $('#19_ceiling').val() : 0;
                            summ = (
